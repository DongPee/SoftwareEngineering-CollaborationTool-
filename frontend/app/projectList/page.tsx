...
export default function ProjectSelector() {
  ...
  const projectList = async () => {
    ...
    try {
      const response = await fetch(
        "http://43.203.124.34:5001/api/showProjects",
        {
          method: "POST",
          headers: {
  ...
  const addProject = async () => {
    ...
    try {
      const response = await fetch(
        "http://43.203.124.34:5001/api/createProject",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteProject = async (projectId: number) => {
    try {
      const response = await fetch(
        "http://43.203.124.34:5001/api/deleteProject",
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateName = (index: number, name: string) => {
    setProjects((prev) =>
      prev.map((proj, i) => (i === index ? { ...proj, name } : proj))
    );
  };

  const updateDesc = (index: number, desc: string) => {
    setProjects((prev) =>
      prev.map((proj, i) => (i === index ? { ...proj, desc } : proj))
    );
  };
  ...
  const handleEditKeyDown = async (
    ...
    if (e.key === "Enter") {
      ...
      try {
        const response = await fetch(
          "http://43.203.124.34:5001/api/updateProject",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            ...
            {projects.map((project, index) => (
              ...
              >
                {project.editing ? (
                  ...
                  <>
                    <span
                      className={`${styles.projectName} text-lg font-semibold m-4`}
                    >
                      {project.name}
                    </span>
                    <span className={`${styles.projectName} text-sm ml-4 mb-2`}>
                      {project.desc}
                    </span>
                  </>
                )}
                <div className="flex gap-2">
                  ...
                  <Link
                    href={`/${
                      project.id
                        ? `project/${project.id}/${encodeURIComponent(
                            project.name
                          )}/${encodeURIComponent(project.desc)}`
                        : "projectList"
                    }`}
                  >
